...
public class EngineParallele {
	...
	public void execute(String filename, Object target, HashMap tags) throws Exception{
		...
		do {
			...
			outerloop:
			for(Sequence sequence : p.getSequences()) {
				
				// Cas de la première séquence
				Sequence premiereSequence = (Sequence) p.getDebut().getReference();
				if(sequence == premiereSequence && !premiereDone) {
					p.getSequencesActives().add(sequence);
			...
			RunnablePara paraRunnable = new RunnablePara(p.getSequencesActives(), target, tags);
